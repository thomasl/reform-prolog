...
solution([], State, _, State) :-
 	length(State, Cnt),
 	format(user_output, '~d elts total~n', [Cnt]),
	true.
solution([N|New0], State0, Table, Sol) :-
 	length(State0, Cnt),
 	format(user_output, '~d elts so far~n', [Cnt]),
	gen(N, N1, N2, N3, N4, Table),
	all_products(New0, Table, Ps0, []),
	sort([N1,N2,N3,N4|Ps0], Ps),
	merge_new(State0, Ps, State, New),
	solution(New, State, Table, Sol).

% :- parallel([all_products/4]).

all_products([], _) --> [].
all_products([X|Xs], Table) --> [X1,X2,X3,X4],
	{gen(X, X1, X2, X3, X4, Table)},
	all_products(Xs, Table).

/* merge_new(A, B, union(A,B), set_diff(B,A)). */

merge_new([], Set, Set0, Set1) :- !, Set = Set0, Set = Set1.
merge_new(Set, [], Set0, Set1) :- !, Set = Set0, Set1 = [].
merge_new([O|Os], [N|Ns], Set, New) :-
	compare(C, O, N), 
	merge_new(C, O, Os, N, Ns, Set, New).
	
merge_new(<, O, Os, N, Ns, Set, New) :- !,
	merge_lt(Os, O, N, Ns, Set, New).
merge_new(=, O, Os, N, Ns, Set, New) :- !,
	Set = [N|Set1],
	merge_new(Os,Ns,Set1,New).
merge_new(>, O, Os, N, Ns, Set, New) :-
	merge_gt(Ns, O, Os, N, Set, New).

merge_lt([], O, N, Ns, Set, New) :- !, Set = [O,N|Ns], New = [N|Ns].
merge_lt([P|Ps], O, N, Ns, Set, New) :- !,
	Set = [O|Set1],
	compare(C,P,N),
	merge_new(C,P,Ps,N,Ns,Set1,New).

merge_gt([],O,Os,N,Set,New) :- !,
	Set = [N,O|Os],
	New = [N].
merge_gt([N|Ns],O,Os,N1,Set,New) :-
	Set = [N1|Set1],
	New = [N1|New1],
	compare(C,O,N),
	merge_new(C,O,Os,N,Ns,Set1,New1).
